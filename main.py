...
class ConnectionManager:
    ...
    async def disconnect(self, websocket: WebSocket, room: str):
        if room in self.active_connections:
            if websocket in self.active_connections[room]:
                ...
                if len(self.active_users[room]) == 0 and room != "ped_pong":
    ...
    async def move_users_to_ped_pong(self, room: str):
        if room == "ped_pong" or not self.active_connections.get(room):
            return
        ...
        # Move users to ped_pong
        for websocket, username in users_to_move:
            try:
                await websocket.send_text(f"System: Moving all users to ped_pong...")
    ...
    async def start_room_transition_timer(self, room: str):
        if room == "ped_pong" or self.available_rooms[room].get("is_special"):
            return
        ...
        try:
            ...
            await self.broadcast(f"System: Room will transition to ped_pong in 20 seconds!", room)
    ...
    def get_time_remaining(self, room: str) -> Optional[int]:
        if room not in self.available_rooms or room == "ped_pong":
            return None
